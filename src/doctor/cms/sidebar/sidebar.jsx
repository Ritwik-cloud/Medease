...
import { fetchDoctorLogout } from "@/redux/doctor/authSlice/doctorAuthSlice";
...
const DoctorSidebar = ({
  ...
  const menuItems = [
    { name: "Dashboard", path: "/doctor/cms/dashboard", icon: Home },
    { name: "Available Timing", path: "available-timing", icon: Stethoscope },
  ...
  const handleLogout = async () => {
    try {
      ...
      const response = await dispatch(fetchDoctorLogout()).unwrap();
            ...
            {menuItems.map((item) => {
              ...
              return (
                <NavLink
                  key={item.name}
                  to={item.path}
                  end={item.path === "/doctor/cms/dashboard"}
                  onClick={() => {
